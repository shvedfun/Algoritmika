...
def create_a_contact(con_name: str = Form(), con_phone: str = Form(), db: Session = Depends(get_async_session)):
...
def create_a_message(mess_text: str = Form(), contact_id: int=Form(), db: Session = Depends(get_async_session), back_task: BackgroundTasks=None): #
...
def create_a_contact(contact: ContactCreate, db: Session = Depends(get_async_session)):
    return create_contact(contact=contact, db=db)
...
def get_a_contact(contact_id: int, db: Session = Depends(get_async_session)):
    return read_contact(contact_id=contact_id, db=db)


@router.get("/contact/{contact_id}/html", response_class=HTMLResponse)
def get_a_contact_html(request: Request, contact_id: int, db: Session = Depends(get_async_session)):
...
def get_messages(
        request:Request, offset: int = 0, limit: int = Query(default=100, lte=100), db: Session = Depends(get_async_session),
):
...
def update_a_contact(contact_id: int, contact: ContactUpdate, db: Session = Depends(get_async_session)):
